    ...
    try:
        while True:
            ...
            while True:
                ...
                img_without_bg = cv2.bitwise_and(img, img, mask=fg_mask)

                img_without_bg = detector.findPose(img_without_bg)
                lmList, bboxInfo = detector.findPosition(img_without_bg, bboxWithHands=False)
                ...
                if bboxInfo:
                    ...
                    y1, y2 = max(0, y_offset), min(img_without_bg.shape[0], y_offset + shirtHeight)
                    x1, x2 = max(0, x_offset), min(img_without_bg.shape[1], x_offset + shirtWidth)
                    ...
                    if y2 - y1 > 0 and x2 - x1 > 0:
                        for c in range(0, 3):
                            img_without_bg[y1:y2, x1:x2, c] = (
                                imgShirtResized[y1_shirt:y2_shirt, x1_shirt:x2_shirt, c]
                            )

                cv2.imshow("Image", img_without_bg)
